...
- Stack: `claude-agent-sdk` 0.2.152 (not yet installed), `pydantic` 2.13.4 (already present as a `google-genai` transitive dependency), `google-genai` 2.23.0 as actually installed (not the stale "2.19.0"/"2.19" figure in older docs), Python 3.11 via `kayak-video`.
