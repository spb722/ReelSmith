...
**Always:** `visual_agent` runs only once a validated `FinalStoryPlanContract` **and** a validated `AnalyzedAssetsContract` both exist (the latter is a hard gate, not just a content source -- missing or invalid `AnalyzedAssetsContract` halts at `visual_agent` before any Claude call, exactly like the `FinalStoryPlanContract` gate). Shots correspond 1:1 to `FinalStoryPlanContract`'s scenes by sequence (AD-2 -- no independent timing source exists pre-audio). Every result validates against `VisualPlanContract` -- schema shape **and** the numeric quality thresholds (`source_fidelity`, `generation_mode_appropriateness`, `visual_coherence`, `narrative_alignment`, `internal_consistency`, each required and >=8, `source_fidelity`/`internal_consistency` >=9 -- freshly designed this story to mirror `story_agent`'s pattern, since `visual_director.py` has no scored-reviewer precedent), both enforced as pydantic validators (AD-3, "passing validation is passing the quality bar") -- reusing `run_bounded_agent_stage` (Story 1.3), ceiling 4. `generation_mode` must be deterministically consistent with `visual_treatment` (AD-4: `AI_VIDEO_CANDIDATE`/`MIXED` may be `VEO`; all other treatments must be `STILL`). Ceiling exhaustion halts via the shared failure-report path (stage id `"visual_agent"`). A persisted, valid, current-sources `metadata/visual_plan.json` (`VisualPlanContract`) skips `visual_agent` entirely and is left byte-identical -- "current-sources" means both structurally current (shot sequences/asset ids match the live `FinalStoryPlanContract`) **and** content-current (AD-7, renegotiated below): a scene's narration/`visual_intent`/`suggested_visual_treatment` changing under an unchanged sequence/asset-id must not skip.

**Never:** Implement `veo_agent`/`stills_agent` (Epic 2) or `voice_agent`/full cross-stage resume (1.5). Modify `visual_director.py`, `prepare_veo_seed_images.py`, `generate_veo_clips.py`, or `prepare_remotion_stills.py` in place. Call Gemini or Veo for planning or generation -- `visual_agent` only assigns metadata (AD-1). Reference subtitle cue ids or audio-derived start/end timestamps (AD-2). Feed `metadata/visual_plan.json` to `prepare_veo_seed_images.py`, `generate_veo_clips.py`, or `prepare_remotion_stills.py` -- it deliberately omits `start_seconds`/`end_seconds`/`primary_subtitle_cue_ids` those scripts require (AD-2); a later adapter story must bridge this.

**Renegotiated (post-review, 2026-09-17):** Added AD-7 (content-staleness guard, AD-2 above) and the explicit `AnalyzedAssetsContract` hard-gate wording -- both already matched the actual shipped code's `run_visual_stage` gating; AD-7's staleness check itself is new (`VisualPlanContract.validate_sources` previously only checked structural sequence/asset-id match, not scene-content equality).
...
| Persisted valid, current-sources `metadata/visual_plan.json` exists | Re-invocation, same reel, no narration/asset changes | `visual_agent` skipped | Proceeds on persisted contract, file left byte-identical, no new spend |
| Persisted `metadata/visual_plan.json` lacks `produced_by`, or shot sequences/asset ids don't match the current `FinalStoryPlanContract`, or scene content changed under an unchanged sequence/asset-id (AD-7) | Legacy `visual_director.py` output, narration/asset changes, or a same-shape content edit | Treated as no persisted contract | `visual_agent` runs for real |

</frozen-after-approval>

## Code Map

- `visual_director.py:96-249` -- `RESPONSE_SCHEMA`: shape reference only. Its `visual_treatment` enum (`USE_EXISTING_ART`/`CROP_AND_RECOMPOSE`/`SUBTLE_ANIMATION`/`TEXT_LED`/`AI_VIDEO_CANDIDATE`/`MIXED`) matches `Scene.suggested_visual_treatment` exactly, reused in `Shot.visual_treatment`; its `start_seconds`/`end_seconds`/`primary_subtitle_cue_ids`/`asset_utilization` are legacy-only and not ported (AD-2).
- `visual_director.py:374-466` (`validate_shots`) -- sequence-continuity and known-asset-id checks are the portable mechanical checks ported into `VisualPlanContract`; the time-contiguity/subtitle-coverage checks are not portable (AD-2). No `MAX_ITERATIONS`/quality-score loop exists here (unlike `story_quality_loop.py`) -- `visual_agent`'s `quality_review` dimensions are freshly designed, mirroring `story_agent`'s pattern (see Boundaries & Constraints).
- `metadata/visual_plan.json` (real, pre-existing manual run, 8 shots) -- schema-shaped but has no `produced_by`; same silent-resume risk Stories 1.2/1.3 already discovered, closed by `VisualPlanContract.produced_by: Literal["visual_agent"]` (required, no default) (AD-6). Now also the live resume checkpoint `run_visual_stage` reads/writes.
- `orchestrator/contracts/final_story_plan.py` -- read-only input; `Scene.sequence`/`source_asset_ids`/`visual_intent`/`suggested_visual_treatment`/`estimated_duration_seconds` drive per-shot planning, the 1:1 shot-to-scene mapping (AD-2), and (AD-7) the content-staleness comparison in `validate_sources`.
- `orchestrator/contracts/analyzed_assets.py` -- read-only input, and (per the Always clause above) a hard existence/validity gate in `run_visual_stage`; `AnalyzedAsset.production` (`story_art_region`, `ui_regions`, `recommended_crop_strategy`, `suggested_motion`, `vertical_video_suitability`) is the deterministic visual metadata `visual_agent` reasons over for frame composition/motion.
- `orchestrator/agents/story_agent.py` -- the pattern `orchestrator/agents/visual_agent.py` mirrors exactly: zero tools, `system_prompt=` on the top-level session (never `agents=`/`extra_args`), `output_format` json_schema, `max_buffer_size` raised, remaining-budget accounting.
- `orchestrator/run.py` -- `run_bounded_agent_stage` (built in Story 1.3) generalizes the retry/resume/halt shape; `run_visual_stage` uses it, gated on validated `FinalStoryPlanContract` + `AnalyzedAssetsContract`, wired into `main()` after the narration stage succeeds.

## Tasks & Acceptance

**Execution:**
- [x] `orchestrator/contracts/visual_plan.py` -- `VisualPlanContract` (pydantic): `produced_by: Literal["visual_agent"]`, `overall_visual_style: str`, `shots: list[Shot]` (min_length=1), `quality_review` (mirrors `QualityReview`'s shape/threshold pattern with the five dimensions fixed in Boundaries & Constraints); model_validators enforce internal shot-sequence continuity (1..N, no gaps), `generation_mode`/`visual_treatment` consistency (AD-4), and per-shot `source_asset_ids` non-empty -- note: exact 1:1 correspondence to `FinalStoryPlanContract`'s *current* scene count/content is enforced only by `validate_sources` below (it needs the story plan as an argument, which a self-contained `model_validate()` doesn't have access to); `run_visual_stage` always calls both together for every real attempt.
- [x] `orchestrator/contracts/visual_plan.py` -- `validate_sources(story_plan)` method: shot sequences must exactly match the current `FinalStoryPlanContract`'s scene sequences (not merely a subset -- one shot per scene is required), each shot's `source_asset_ids` must be a subset of that scene's own `source_asset_ids`, **and** (AD-7, new) each shot's referenced scene's `narration`/`visual_intent`/`suggested_visual_treatment` must match what the shot was actually generated against -- store a comparable snapshot (e.g. a per-shot content fingerprint) at generation time and compare it on resume, so a same-shape content edit cannot silently skip.
...
- [x] `orchestrator/tests/test_visual_agent.py` -- load the real, pre-existing `metadata/visual_plan.json` from disk (not a synthetic fixture) and assert `VisualPlanContract.model_validate` rejects it (missing `produced_by`) -- proves the provenance-gate closes the actual legacy artifact this story exists to reject, not just a hand-built stand-in.
- [x] `orchestrator/contracts/visual_plan.py` + `orchestrator/tests/test_visual_plan_contract.py` -- AD-7 content-staleness guard: a test where sequences/asset-ids match but scene narration/visual_intent/suggested_visual_treatment changed must be rejected by `validate_sources`.
...
- Given a validated `FinalStoryPlanContract` but no validated `AnalyzedAssetsContract` (or an invalid one), when the orchestrator reaches `visual_agent`, then it halts via the failure-report path before any Claude call.
...
- Given a valid, current-sources `metadata/visual_plan.json`, when the orchestrator re-invokes, then `visual_agent` is skipped and the file is left byte-identical.
- Given a persisted `metadata/visual_plan.json` with a missing/wrong `produced_by`, or stale sources (structural or content, AD-7), when the orchestrator invokes, then it is treated as absent and `visual_agent` runs for real.
...
- `QualityReview` mirrors `FinalStoryPlanContract`'s shape but renames
  ...
  Boundaries & Constraints.
- AD-7's content fingerprint (`Shot.scene_content_fingerprint`, a sha256 of
  `narration`/`visual_intent`/`suggested_visual_treatment`) is typed
  `SkipJsonSchema[str] = ""`, which excludes it from
  `VisualPlanContract.model_json_schema()` entirely -- `visual_agent` never
  sees or sets it, so a freshly parsed attempt from Claude always starts
  with `""`. `validate_sources` treats `""` as "not yet stamped" (fresh
  attempt: it stamps the real fingerprint from the current scene, in
  place, before `run_bounded_agent_stage` persists it) and any non-empty
  value as "loaded from a prior persist" (compared, not overwritten, and
  raises on mismatch). This avoids needing to know, from inside the shared
  `run_bounded_agent_stage` kernel (reused as-is, not modified), whether a
  given `validate_contract` call is the persisted-file check or a live
  attempt's post-validation check.
...
**Pass 2** (external post-hoc review; verdicts: 1 medium [real gap], 1 low, 5 doc-only/already-true, 3 false/already-covered)

- `orchestrator/contracts/visual_plan.py` (`validate_sources` checks only structural sequence/asset-id match, not scene *content*) — **medium, real** — a regenerated `FinalStoryPlanContract` with the same scene count/asset-ids but changed narration/visual_intent/suggested_visual_treatment would wrongly satisfy resume-skip. → **patch** (AD-7 above).
- `orchestrator/tests/test_visual_agent.py` (no test loads the actual real `metadata/visual_plan.json` through the resume path; only synthetic legacy-shaped fixtures) — **low, real** — → **patch** (Task added above).
- `run_visual_stage` gates on `AnalyzedAssetsContract` — **already true in code**, just previously undocumented in frozen Boundaries/matrix — → **patch (doc only)**, done above.
- Byte-identical-skip / reject-stale-or-legacy ACs — **already implemented and tested** (`test_valid_persisted_contract_skips_claude`, `test_invalid_or_stale_persistence_cannot_skip`) — → **patch (doc only)**: made explicit as ACs above.
- Scene-count correspondence attributed to "validators" generally rather than `validate_sources` specifically — reworded the Tasks bullet above (this exact confusion recurred with a second, independent reviewer, even though Pass 1 found no demonstrated bad outcome).
- `metadata/visual_plan.json` not named in the frozen matrix; Code Map in future tense — **low, real, cosmetic** — → **patch (doc only)**, done above.
- Missing-timing-fields operator warning — **already logged in `deferred-work.md`**; also now stated directly in this spec's Never clause above for visibility.
- "Cited asset IDs must exist in the current analyzed-assets set" (direct check requested on `VisualPlanContract` itself) — **false** — already guaranteed transitively: `main()` re-validates `FinalStoryPlanContract` against the live `AnalyzedAssetsContract` on every invocation before `visual_agent` is ever reached, so by the time `run_visual_stage` runs, the story plan's (and therefore every shot's) asset ids are already current.
- Quality-bar "self-scored ≥8/≥9 enough?" and "VEO ratio / SUBTLE_ANIMATION+STILL/MIXED meaning for Epic 2" — **out of scope** — the first is the same accepted tradeoff already documented in Design Notes (Story 1.3's identical pattern); the second is explicitly Epic 2's concern per this story's own Never section.
